...
func trackingBehind(
	...
	switch {
	case earliestGap == "" && len(applied) == 0:
		// Схема полная, а учёт пуст — накатывали целиком мимо него.
		return all[len(all)-1].Version, true
	case earliestGap == "":
		// Схема полная, учёт неполон — но это ещё НЕ повод советовать
		// отметку: сверка знает не про каждую миграцию (0016, например,
		// только приводит в порядок имена ограничений и никаких новых
		// объектов не вводит). Обычное обновление выглядит точно так же, и
		// посоветовать здесь --baseline значило бы предложить ПРОПУСТИТЬ
		// нужную миграцию — ошибка дороже той, от которой мы защищаемся.
		//
		// Если учёт всё-таки отстал, это выяснится при применении, и
		// сообщение упавшей миграции несёт ту же подсказку.
		return "", false
