...
Сервис `api-go` держит фоновый наблюдатель: раз в `SCAN_WATCHER_INTERVAL_SECONDS`
он ищет пакеты заявок, у которых шаг скачивания пройден, а отчётов нет, и
прогоняет по ним сканеры. Ничего запускать руками не нужно — отчёт появляется в
карточке пакета через минуту после того, как заявка дошла до скачивания.

Выборка намеренно узкая: пакеты, застрявшие до скачивания (blacklist, карантин,
нет в реестре), не берутся — сканировать там нечего. Пакет с уже готовым
отчётом не берётся тоже, иначе он сканировался бы по кругу на каждом тике.
Упавший прогон отчёта не создаёт, поэтому пакет попадёт в выборку снова — это и
есть весь механизм повторов.

Выключается через `SCAN_WATCHER_ENABLED=false`.

Наблюдатель — конструкция ПЕРЕХОДНОГО периода. Когда Go получит собственный API
заявок и конвейер целиком, сканирование станет обычным шагом, а наблюдатель
уедет.

### Отчёты по существующей заявке: `moderation scan`

Обычно не нужна — наблюдатель выше делает это сам. Команда пригодится, чтобы
пересканировать пакет принудительно (сменили правила, поставили semgrep) или
разобрать конкретную заявку не дожидаясь тика.

```bash
# все пакеты заявки — номер виден в адресе её страницы: /requests/42
docker compose exec api-go moderation scan --request 42

# либо один пакет
docker compose exec api-go moderation scan --item 108
```

`--request` — номер заявки (`moderation_request.id`), тот же, что в адресе
страницы. `--item` — идентификатор ОДНОГО пакета внутри заявки
(`request_item.id`), он есть в ответе `GET /api/v1/requests/{id}` в поле
`packages[].id`. В обычной работе удобнее `--request`: искать идентификаторы
пакетов не нужно.
...
После прогона отчёты видны **в карточке пакета** в веб-интерфейсе — блок
«Отчёты о сканировании» со ссылками на HTML и JSON. Блок появляется только при
наличии отчётов: если Go-сервис не поднят или сканирование не запускалось,
карточка выглядит как прежде, без ошибок.
...
# целиком, с базой — обязательно -p 1
createdb moderation_test
for f in migrations/*.up.sql; do psql -d moderation_test -v ON_ERROR_STOP=1 -f "$f"; done
MODERATION_TEST_POSTGRES_DSN='postgres://localhost/moderation_test' go test -p 1 ./...
```

`-p 1` — не перестраховка. База одна на все пакеты, а часть тестов проверяет состояние
СЕРВИСА целиком, а не своей строки: «после захвата очередь пуста», «активная версия снапшота
одна». При параллельном прогоне пакетов такие проверки видят чужие строки и падают — то есть
ломается не код, а тест, и разбираться приходится в самом неудачном месте. Без базы
параллельность безопасна.
