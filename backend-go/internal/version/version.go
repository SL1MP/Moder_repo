...
//
// Почему своя реализация, а не библиотека: экосистемы используют разные
// наборы правил (semver, PEP 440, NuGet-интервалы, MVS у Go и диапазоны
// Conan), и одна библиотека их не закрывает. Границы реализованного описаны у
// каждой схемы честно: там, где правило упрощено, это сказано в комментарии,
// а не скрыто за «обычно работает».
package version
...
// selectHighest — общий выбор для npm, pypi и Conan: максимальная подходящая
// версия, пререлизы в расчёт не идут, пока их не потребовало само требование.
//
