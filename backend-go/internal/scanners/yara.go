...
func (s YaraScanner) Scan(ctx context.Context, root string) (Outcome, error) {
	...
	// Флаги: -s печатать совпавшие строки, -L длину совпадения, -r рекурсивно
	// по каталогу, -w без предупреждений (иначе предупреждения компиляции
	// правил перемешиваются с результатами в stdout), -N не следовать по
	// символическим ссылкам (их распаковщик и так не создаёт, но каталог
	// сканирования может оказаться и не от него).
