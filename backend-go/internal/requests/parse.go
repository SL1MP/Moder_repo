...
// transitiveWarning — один и тот же текст, что и у python-версии.
const transitiveWarning = "Проверяется и публикуется только сам заявленный пакет. " +
	"Транзитивные зависимости автоматически не подтягиваются — если они нужны, " +
	"заведите их отдельно."
...
var indeterminateKindFiles = map[string][]string{"go": {"go.sum"}}
...
func (s *Service) fromFile(plugin registry.Plugin, in Input) ([]entry, []string, error) {
	...
	if isIndeterminateKind(plugin.Code(), in.Filename) {
		warnings = append(warnings, fmt.Sprintf(
			"Файл «%s» перечисляет весь граф модулей: и прямые зависимости, и транзитивные — "+
				"формат их не различает. Приняты все записи; лишние удалите из заявки вручную.",
			in.Filename))
	}
