    ...
    *,
    ...
    if content is not None:
        ...
        if _kind_is_indeterminate(plugin, filename or ""):
            # go.sum, poetry.lock, yarn.lock: признака прямой зависимости в
            # формате нет вовсе, поэтому все записи принимаются как прямые.
            # Только предупреждения здесь было мало: фильтр транзитивных
            # выбрасывал весь файл, и заявка приезжала пустой.
            for entry in parsed:
                if entry.ref is not None:
                    entry.ref.dependency_kind = "direct"
            result.warnings.append(
                f"Файл «{filename}» перечисляет закрытый набор зависимостей и не различает "
                "прямые и транзитивные — формат такого поля не имеет. Приняты все записи; "
