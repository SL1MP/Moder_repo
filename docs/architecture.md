...
| 7 | `sast_scan` | статический анализ исходников (`semgrep`) | `warn` → `awaiting_security` при находках не ниже `SAST_MIN_SEVERITY` |
...
Оба шага конвейер не останавливают (`StepOutcome.pending`, флаг `defer`), чтобы DevSecOps увидел
все находки разом, а не по одной за прогон; при этом уведомление о находке уходит сразу по ходу
прогона, не дожидаясь его завершения. Одно его решение закрывает все три свои проверки —
уязвимости, баннеры и SAST, — и записывается на версию пакета (`security_override_*`),
иначе возобновлённый конвейер нашёл бы то же самое и заблокировал публикацию снова. Проверка
`security_override_at` реализована в каждом из трёх шагов по отдельности (`app/pipeline/steps.py`),
не централизованно — при добавлении нового шага сканирования содержимого эту проверку нужно
скопировать явно, иначе он будет игнорировать уже принятое DevSecOps решение и блокировать
повторно.
