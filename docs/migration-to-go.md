...
8. Адаптеры: `ArtifactStore` — **перенесены обе реализации**. Сначала только `generic`
   (JFrog Artifactory), по приоритету из `docs/ci-parity-gaps.md` ("Артефактори — production это
   JFrog Artifactory, не Nexus"); `nexus` пришлось перенести следом, когда стенд заказчика
   оказался на Nexus и каждая публикация падала с 405: PUT по адресу файла Nexus не принимает,
   выгрузка идёт компонентным API. Тип выбирается `ARTIFACT_STORE`, значение по умолчанию —
   `nexus`, как в python-версии и `.env.example`. Плюс решить модель публикации (тот же документ: copy-в-
