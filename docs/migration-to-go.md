...
5. 🔶 **REST API: маршруты чтения** — перенесены `GET /managers`, `GET /managers/detect`,
   ...
   вместе с созданием.
   **Ещё не перенесено:** решения ролей (`/items/{id}/...`), `POST /requests/{id}/retry`,
   админка, GitLab, `POST /packages/check`.
...
11. REST API контракт — сверка с текущим `docs/api.md` и живым фронтендом (frontend не
   переписывается, только конфигурация вызовов, если понадобится).
12. Отключение Python-стека, `docker-compose.yml` этого репозитория переезжает на Go-образы;
   `backend/` (Python) архивируется, не удаляется молча — см. открытый вопрос ниже.
13. **Конфигурация — admin API + БД вместо `.env`+рестарт** (прямой запрос пользователя, см.
