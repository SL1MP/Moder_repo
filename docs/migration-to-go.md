...
9. 🔶 **Воркер: очередь на Postgres вместо Celery/Redis** — сделано ядро: `internal/queue`
   ...
   **Отход от исходного плана: брокера нет, ни NATS, ни Valkey.** Очередь — это строки
   ...
   Go-воркер поднимается профилем `go-worker` и по умолчанию выключен: пока живы оба, пакет
   достаётся тому, кто первым захватил, а поведение отличается в мелочах. Переключение —
   `make go-worker` / `make py-worker`.

   **Ещё нет:** постановки задач из API при решениях ролей (их по-прежнему ведёт
   python-версия), сторожа зависших заявок как отдельного процесса (его роль пока играет
   перехват брошенных прогонов самим воркером).
