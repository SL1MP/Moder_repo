...
такого поля не имеют вовсе — их записи принимаются целиком, с предупреждением.

При `include_transitive=false` (по умолчанию) в заявку попадают только прямые зависимости.
При `include_transitive=true` сервис ещё и **раскрывает дерево по данным реестра**: каждая
найденная зависимость приводится к точной версии и заводится отдельным пакетом заявки со
ссылкой на родителя. Глубина — `resolve_depth` (по умолчанию `RESOLVE_MAX_DEPTH`, больше
настроенной попросить нельзя).

### Предпросмотр дерева зависимостей

`POST /api/v1/dependencies/resolve` показывает, что заявка потянет за собой, **до** её создания —
ничего не создаёт и ничего не пишет.

```bash
curl -sS -X POST http://localhost:8080/api/v1/dependencies/resolve \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"manager":"npm","packages":["express@4.18.2"],"depth":2}'
```
...
```json
{
  ...
  "depth": 2,
  "truncated": false,
  "summary": "транзитивных зависимостей: 48; глубина: 2; конфликтов версий: 1",
  "limits": {"max_depth": 3, "max_packages": 200},
  "packages": [
    {"name": "express", "version": "4.18.2", "state": "new", "depth": 0},
    {"name": "accepts", "version": "1.3.8", "state": "new", "depth": 1,
     "required_by": "express 4.18.2", "required_range": "~1.3.8"}
  ],
  "conflicts": [{"name": "ms", "versions": ["2.0.0", "2.1.3"]}],
  "problems": []
}
```

Три поля отвечают за то, чего в дереве **нет**, и молчать о них нельзя — неполное дерево выглядит
точно так же, как полное:

| Поле | Что значит |
| --- | --- |
| `truncated` | упёрлись в предел (`limits.max_packages`), показана часть дерева |
| `problems[]` | зависимость не раскрыта: нет версии под требование, реестр не ответил, требование не разобрано |
| `conflicts[]` | один пакет затребован в разных версиях; в заявку попадут обе — выбор за разработчиком |

`resolved: false` означает «менеджер пока не умеет раскрывать зависимости», а не «зависимостей
нет»: раскрытие есть у pypi, npm, go и nuget.
...
не накатили миграцию. Что именно — покажет `docker compose run --rm migrate-go schema`.
