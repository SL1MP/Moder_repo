...
### `migrate-go` падает: «check constraint … is violated by some row»

Стенд, на котором схему накатывали ДО появления учёта миграций — прежняя
инструкция предлагала прогнать файлы циклом через `psql`, и такой прогон
ничего о себе не записывал. Таблица `schema_migrations_go` пуста, `migrate`
начинает с первой миграции и упирается не в схему, а в ДАННЫЕ: миграция 0007
добавляет CHECK на статусы `request_item` без `cancelled`, потому что этот
статус появляется только в 0011. Если заявки на стенде уже закрывали, строки
...
Начиная с этой версии `migrate` распознаёт такое состояние сам: вместо падения
на ограничении он останавливается и печатает готовую команду с нужной версией.

**Сначала пересоберите образ**: после `git pull` в контейнере лежит прежний
бинарник — ни распознавания, ни `--baseline` в нём нет, и на флаг он отвечает
«flag provided but not defined». Исходники обновляет git, образ — только
...
есть. Отметить ЛИШНЮЮ хуже, чем ошибиться в меньшую сторону: отмеченная, но не
выполненная миграция не применится никогда, и обнаружится это на первом
обращении к недостающему столбцу. Поэтому шаг 3 со сверкой обязателен — она
сравнивает базу с тем, что пишет код.
